...
export const verifyAndCreateUser = async (req, res) => {
  ...
  try {
    ...
    let idPin = Math.floor(10000 + Math.random() * 90000).toString();

    let isExistUserWithId = await User.findOne({ idPin });

    while (isExistUserWithId) {
      idPin = Math.floor(10000 + Math.random() * 90000).toString();
      isExistUserWithId = await User.findOne({ idPin });
    }
